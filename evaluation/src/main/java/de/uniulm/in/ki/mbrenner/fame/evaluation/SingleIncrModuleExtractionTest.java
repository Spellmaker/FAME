...
public class SingleIncrModuleExtractionTest implements EvaluationCase{
    ...
    public void evaluate(List<File> ontologies, List<String> options) throws Exception {sig_size = Integer.parseInt(options.get(0));
        ...
        int sizecount = 0;
        int errors = 0;
        String header = "file;axioms;logical axioms;base module;incr";
        lines.add(header);
        for(File f : ontologies){
            ...
            try{
                ...
                try {
                    ...
                    EvaluationMain.out.println("Incr built");
                    long start = System.currentTimeMillis();
                    for (Set<OWLEntity> sign : signatures) {
                        IncrementalModule im = ie.extractModuleStatic(sign);
                    }
                    long end = System.currentTimeMillis();
                    line += (end - start);
                }
                catch(Throwable t){
                    EvaluationMain.out.println("Incr error: " + t);
                    errors++;
                    line += -1;
                }
                lines.add(line);
