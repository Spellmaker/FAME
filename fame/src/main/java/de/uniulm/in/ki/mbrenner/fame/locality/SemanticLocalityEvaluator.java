 ...
 */
package de.uniulm.in.ki.mbrenner.fame.locality;
...
public class SemanticLocalityEvaluator implements LocalityEvaluator {
    ...
    private class AxiomLocalityVisitor extends OWLAxiomVisitorAdapter implements
        ...
        public void visit(OWLEquivalentClassesAxiom axiom) {
            ...
            if (eqClasses.size() != 2) {
            	if(eqClasses.size() == 1 && eqClasses.iterator().next().isBottomEntity()){
                	isLocal = true;
                }
                return;
            }
    ...
    private class BottomReplacer extends OWLAxiomVisitorAdapter implements
            OWLAxiomVisitor, OWLClassExpressionVisitor, OWLPropertyExpressionVisitor {

        /** The new axiom. */
        private OWLAxiom newAxiom;
        /** The new class expression. */
        private OWLClassExpression newClassExpression;
        /** The new object property expression */
        private OWLObjectPropertyExpression newObjectPropertyExpression;
        /** The signature. */
        ...
        public OWLObjectPropertyExpression replaceBottom(OWLObjectPropertyExpression desc){
        	newObjectPropertyExpression = null;
        	desc.accept(this);
        	if(newObjectPropertyExpression == null){
        		throw new RuntimeException("Unsupported object property expression " + desc);
        	}
        	return newObjectPropertyExpression;
        ...
        @Override
        public void visit(OWLAnnotationProperty ann){
        ...
        public void visit(OWLSubObjectPropertyOfAxiom ax){
        	OWLObjectPropertyExpression sup = replaceBottom(ax.getSuperProperty());
        	OWLObjectPropertyExpression sub = replaceBottom(ax.getSubProperty());
        	
        	newAxiom = df.getOWLSubObjectPropertyOfAxiom(sub, sup);
        }

		@Override
		public void visit(OWLObjectProperty property) {
			if(signature.contains(property)){
				newObjectPropertyExpression = property;
			}
			else{
				newObjectPropertyExpression = df.getOWLBottomObjectProperty();
			}
		}
